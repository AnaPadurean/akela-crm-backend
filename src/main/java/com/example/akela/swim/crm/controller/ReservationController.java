...
public class ReservationController {
    ...
    public ReservationController(ReservationService reservationService, CoachService coachService, ChildrenService childrenService, SessionService sessionService, SubscriptionService subscriptionService, WeeklyTrainingSessionService weeklyTrainingSessionService) {
    ...
    public ResponseEntity<List<ReservationDTO>> getAllReservations() {
        List<ReservationDTO> dtos = reservationService.findAll().stream().map(r -> {
            ...
            if (r.getSubscription() != null) {
                ...
                dto.setSubscriptionName(r.getSubscription().getPlan().getSubscriptionType().getName() + " (" + r.getSubscription().getPlan().getSessions() + " ședințe)");
            }
    ...
    public ResponseEntity<?> createReservation(@RequestBody ReservationDTO dto) {
        try {
            CoachEntity coach = coachService.findById(dto.getCoachId()).orElseThrow(() -> new RuntimeException("Coach not found"));

            ChildrenEntity child = childrenService.findById(dto.getChildId()).orElseThrow(() -> new RuntimeException("Child not found"));

            SubscriptionEntity subscription = subscriptionService.findById(dto.getSubscriptionId()).orElseThrow(() -> new RuntimeException("Abonamentul selectat nu există"));

            WeeklyTrainingSessionEntity weekly = null;
            if (dto.getWeeklyTrainingSessionId() != null) {
                weekly = weeklyTrainingSessionService.findById(dto.getWeeklyTrainingSessionId()).orElseThrow(() -> new RuntimeException("Weekly session not found"));
            }
    ...
    public ResponseEntity<?> updateReservation(@PathVariable Long id, @RequestBody ReservationDTO dto) {
        return reservationService.findById(id).map(existing -> {
            if (dto.getCoachId() != null) {
                coachService.findById(dto.getCoachId()).ifPresent(existing::setCoach);
            }
            if (dto.getChildId() != null) {
                childrenService.findById(dto.getChildId()).ifPresent(existing::setChild);
            }
            ...
            if (updated.getCoach() != null) {
                ...
                response.setCoachFullName(updated.getCoach().getCoachLastName() + " " + updated.getCoach().getCoachFirstName());
            }
            if (updated.getChild() != null) {
                response.setChildId(updated.getChild().getChildId());
                response.setChildFullName(updated.getChild().getChildLastName() + " " + updated.getChild().getChildFirstName());
            }
    ...
    @GetMapping("/filter")
    public ResponseEntity<List<ReservationDTO>> filterReservations(@RequestParam(required = false) String startDate, @RequestParam(required = false) String endDate
    ...
    public ResponseEntity<List<ReservationDTO>> getByWeekly(@PathVariable Long weeklyId) {
        ...
        List<ReservationDTO> dtos = reservationService.findByWeeklyTrainingSessionId(weeklyId).stream().map(this::toDto).toList();
    ...
    public ResponseEntity<List<ReservationDTO>> getBySubscription(@PathVariable Long subscriptionId) {
        List<ReservationDTO> dtos = reservationService.findBySubscriptionId(subscriptionId).stream().map(this::toDto).toList();
        return ResponseEntity.ok(dtos);
    }
